...
public class WhiteboardServer {
	 ...
	 */
	public static final Map<String,String> keyValueMap=new HashMap<>();

	public static final Set<Endpoint> allConnectedClients= new HashSet<>();
	 ...
	 * @param peerport
	 */
	private static void indexUpdate(String boardId,String peerport) {
		synchronized(keyValueMap) {
			if(!keyValueMap.containsKey(boardId))
			{
				keyValueMap.put(boardId,peerport);
			}
		}
	}
	...
	public static void main( String[] args ) throws IOException, InterruptedException
    {
		...
		serverManager.on(ServerManager.sessionStarted,(eventArgs)->{
			...
			{
				endpoint.emit(sharingBoard,keyValueMap.get(s)+":"+s);
			}
			allConnectedClients.add(endpoint);
        	...
        	endpoint.on(shareBoard, (eventArgs2)->{
        		...
        		if(parts.length!=3) {
        			endpoint.emit(WhiteboardServer.error,update);
        		} else {
	        		String peerport = parts[0]+":"+parts[1];
					indexUpdate(parts[2],peerport);
					for(Endpoint peer:allConnectedClients)
					{
						peer.emit(WhiteboardServer.sharingBoard,update);
					}
        		}
        	}).on(unshareBoard, (eventArgs2)->{
				String query = (String) eventArgs2[0];
        		log.info("Received index update: "+query);
        		String[] parts=query.split(":",3);
				log.info("Received query: "+query);
				if(keyValueMap.containsKey(parts[2]))
				{
					keyValueMap.remove(parts[2]);
				}
				for(Endpoint peer:allConnectedClients)
				{
					peer.emit(WhiteboardServer.unsharingBoard,query);
				}
			...
			allConnectedClients.remove(endpoint);
			Set<String> valuesHaveToRemove=new HashSet();
			for(String s:keyValueMap.keySet())
			{
				String address=keyValueMap.get(s);
				if(address.equals(endpoint.getOtherEndpointId().substring(1)))
				{
					valuesHaveToRemove.add(s);
					
				}
			}
			for(String s:valuesHaveToRemove)
			{
				keyValueMap.remove(s);
			}
			//  Arpan has to remove the boards which are share by this end point
